...
public class CharacterCreator extends Observable  implements Serializable{
    ...
    public String[] getSpecializations() {
        ...
        * */
        int length = Specialization.values().length;
        ...
        for (Specialization sp : specializations) {
            String val = sp.name();
            result[i] = val.substring(0, 1).toUpperCase() + val.substring(1).toLowerCase();
            ++i;
        }
    ...
    public void setSpecialization(int position) {
        ...
        * */
        ...
        if (position > Specialization.values().length) {
    ...
    public void setRace(int position) {
        ...
        * */
        ...
        if (position > Race.values().length) {
...
//logic
        if (mAvailablePoints - updateTo >= 0) {
            ...
            int attrVal = mAttributesMap.get(attrName);
            if ((updateTo > 0 && mAvailablePoints >= updateTo) || (updateTo < 0 && attrVal + updateTo > 0)) {
                mAvailablePoints -= updateTo;
                attrVal += updateTo;
                mAttributesMap.put(attrName, attrVal);
